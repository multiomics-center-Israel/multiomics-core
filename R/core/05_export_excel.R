...
write_final_results_excels_legacy_generic <- function(final_results, config, out_dir, mode, id_col,
    ...
    save_wb_results <- function(df, path, with_cutoffs = FALSE) {
        ...
        if (n_annot_rows > 0) {
            ...
            # in df_out (after potential renaming). The measured, .norm and
            # imp. blocks all belong to the same sample, so the annotation rows
            # must repeat over each block that is present.
            sample_col_map <- do.call(rbind, lapply(sample_cols_in_df, function(sc) {
                ...
                idx <- match(
                    c(display, paste0(display, ".norm"), paste0("imp.", display)),
                    colnames(df_out)
                )
                idx <- idx[!is.na(idx)]
                if (length(idx) == 0L) return(NULL)
                data.frame(sample_id = sc, col_idx = idx, stringsAsFactors = FALSE)
            }))
    ...
    if (!is.null(expr_for_de)) {
        ...
        # expression, group means, observed-only summaries, DE stats,
        # clustering, z-scores, or 'order'
        all_known <- c(id_cols, expr_cols, norm_cols_present, raw_summary_cols,
                       mean_cols_present, cv_cols_present, obs_cols_present,
                       imp_cols_present, de_stat_cols,
...
#'   [Mean/raw summaries], [CV.<group>], [n_obs/obs FC], [imp.<sample>],
#'   DE stats, pass_any_contrast
