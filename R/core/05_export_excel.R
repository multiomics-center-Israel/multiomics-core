...
#'   setting this workbook was produced under. It changes how .norm and
#'   log2FC.imputs are described (averaged over imputation runs, or one fit),
#'   not whether the hand check closes. Ignored for other modes.
...
build_provenance_notes <- function(mode = "rna", multi_imputation = TRUE) {
    ...
    if (identical(mode, "rna")) {
        ...
        # The .norm and Mean. columns are the average of the imputation runs and
        # log2FC.imputs is the mean of the per-run coefficients, so for limma the
        # hand check closes exactly under both imputation settings. The wording
        # differs only in what .norm and log2FC.imputs are averaged over.
        ...
        glossary <- data.frame(
            ...
            Meaning = c(
                ...
                if (multi_imp) {
                    "The same matrix after imputation, averaged over the imputation runs: measured values are unchanged, and each imputed cell is the mean of its draws."
                } else {
                    "The same matrix after imputation. This is the exact matrix limma was fitted on."
                },
                ...
                if (multi_imp) {
                    "Mean over the imputation runs of each run's limma log2 fold change."
                } else {
                    "limma log2 fold change for the contrast."
                },
                "Mean.<numerator> minus Mean.<denominator>, computed from the two Mean cells in this row. Equal to log2FC.imputs when the DE step compares these group means directly (see note 3).",
                fc_rule,
        ...
        notes <- c(
            "Reconciling the fold change from this row:",
            "1. Mean.<numerator> minus Mean.<denominator> is log2FC.imputs. log2FC_from_means is that subtraction done for you.",
            if (multi_imp) {
                "This is exact for limma: each imputation run's coefficient is that run's difference of group means, log2FC.imputs is the mean of those coefficients, and the .norm values are the mean of the runs, so both sides average the same numbers."
            } else {
                "This is exact for limma: its coefficient is the difference of the group means of the matrix it was fitted on, which is the .norm block."
            },
            "2. Apply the linearFC rule above to log2FC.imputs. This step is exact.",
            "3. Where log2FC_from_means and log2FC.imputs differ, the DE step did not compare these group means directly: a paired t-test uses only samples with a matching pair, de$block_col weights samples unevenly when blocks do not hold the same number of samples from each group, and a DE table loaded from outside the pipeline was not fitted on this matrix.",
...
get_contrast_cols <- function(contrast, mode = "proteomics") {
    ...
    if (mode == "rna") {
        ...
        # columns are the average of the imputation runs, so for limma their
        # difference IS log2FC.imputs and a shrinkage check against it would
        # compare a number with itself. The measured-only estimate is the one
        # genuinely free of the model.
        #
        # log2fc_means keeps the real column name; build_final_results_proteomics()
        # emits it under every imputation setting as the hand check.
        list(
