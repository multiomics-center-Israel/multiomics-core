...
add_de_reconciliation_sheet <- function(wb, recon, sheet = "DE_reconciliation") {
    ...
    note <- c(
        ...
        "jensen_gap is log2FC.imputs minus mean.log2FC.runs. The mean of a set of ratios is not the ratio implied by the mean of their logs, so the pooled value sits at or above the mean of the per-run estimates. It is 0 when every per-run estimate that EXISTS is the same value. Runs that produced no estimate do not count: the pooling drops them, and so does mean.log2FC.runs, so a feature only one run could estimate also has a zero gap although its run columns plainly differ. A feature measured in every sample is the common example of a zero gap -- nothing was imputed, so every run saw the same values -- but that is an example, not the condition. The column explains a difference; it is not a statistic to report.",
