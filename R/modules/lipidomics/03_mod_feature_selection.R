# R/modules/lipidomics/03_mod_feature_selection.R
#
# Lipidomics feature selection module: RF + PLS-DA.
# Reuses metabolomics plot helpers: plot_rf_importance, plot_plsda_scores, plot_plsda_vip


#' Lipidomics feature selection module
#'
#' @param pre     List from preprocess_lipidomics().
#' @param config  Full config.
#' @param out_dir Output directory for this mode.
#' @return list(rf, plsda, plots, files) or NULL.
mod_lipidomics_feature_selection <- function(pre, config, out_dir) {
    ...
    # ---- Random Forest ----
    rf_res <- tryCatch(
        run_lipidomics_rf(pre, config),
        error = function(e) {
            warning("lipidomics RF failed: ", e$message)
            NULL
        }
    )
    ...
    if (!is.null(rf_res)) {
        ...
        rf_cfg <- config$modes$lipidomics$rf %||% list()
        ...
        p_rf <- tryCatch({
            ...
            pp
        }, error = function(e) { warning("RF plot failed: ", e$message); NULL })
        if (!is.null(p_rf)) {
    ...
    # ---- PLS-DA ----
    plsda_res <- tryCatch(
        run_lipidomics_plsda(pre, config),
        error = function(e) {
            warning("lipidomics PLS-DA failed: ", e$message)
            NULL
        }
    )

    if (!is.null(plsda_res)) {
        plsda_cfg <- config$modes$lipidomics$plsda %||% list()
        ...
        p_scores <- tryCatch({
            ...
            ps
        }, error = function(e) { warning("PLS-DA scores failed: ", e$message); NULL })
        if (!is.null(p_scores)) {
        ...
        p_vip <- tryCatch({
            ...
            pv
        }, error = function(e) { warning("PLS-DA VIP failed: ", e$message); NULL })
        if (!is.null(p_vip)) {
    ...
    if (is.null(rf_res) && is.null(plsda_res)) {
        message("lipidomics feature selection: both RF and PLS-DA disabled — skipping")
        return(NULL)
    }

    list(rf = rf_res, plsda = plsda_res, plots = plots, files = unique(files))
}
