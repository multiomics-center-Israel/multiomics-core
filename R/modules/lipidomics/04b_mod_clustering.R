# R/modules/lipidomics/04b_mod_clustering.R
#
# Lipidomics clustering module — thin wrapper over core clustering functions.
# Mirrors the metabolomics clustering module shape so the unified Clustering
# tabset (hierarchical / partition / binary) lights up in the lipidomics
# report exactly like it does in metabolomics + proteomics.
#
...
#         save_heatmap_to_file, compute_cluster_gaps,
#         save_per_cluster_heatmaps, save_cluster_profile_outputs


#' Lipidomics clustering module
#'
#' @param pre    Preprocessed lipidomics object (expects $expr_work, $meta).
...
mod_lipidomics_clustering <- function(pre, de_res, config, out_dir) {
    stopifnot(is.character(out_dir), length(out_dir) == 1)

    assert_pre_contract(pre, stage = "lipidomics")
    assert_de_contract(de_res, stage = "lipidomics")
    ...
    bio <- filter_to_biological(pre$expr_work, pre$meta, condition_col,
                                sample_col, label = "lipidomics clustering")
    ...
    if (is.null(cl) || isFALSE(cl$enabled)) {
        message("lipidomics clustering: disabled in config — skipping")
        return(list(plots = list(), files = character(0),
                    excel_order = NULL, objects = list()))
    }

    # ---- Extract DE features ----
    de_features <- get_de_features_lipidomics(de_res)

    if (length(de_features) < 2) {
        message(sprintf(
            "lipidomics clustering: skipped — %d DE features (need >= 2)",
            length(de_features)
        ))
        return(list(plots = list(), files = character(0),
                    excel_order = NULL, objects = list()))
    ...
    message(sprintf("lipidomics clustering: %d DE features", length(de_features)))
    ...
    if (isTRUE(flags$hierarchical)) {
        ...
        # DE row annotations
        de_cfg <- cfg$de %||% list()
        lip_p_cutoff  <- de_cfg$p_cutoff %||% 0.05
        lip_lin_fc    <- de_cfg$linear_fc_cutoff %||% 1.5
        lip_log2fc    <- log2(lip_lin_fc)

        annot_context <- list(
            summary_df    = de_res$summary_df,
            p_cutoff      = lip_p_cutoff,
            log2fc_cutoff = lip_log2fc,
            id_col        = "feature_id"
        )
...
#' Extract DE feature IDs from lipidomics DE results
#'
...
#' @keywords internal
get_de_features_lipidomics <- function(de_res) {
