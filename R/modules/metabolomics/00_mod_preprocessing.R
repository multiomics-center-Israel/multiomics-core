...
mod_met_raw <- function(inp, config) {
  ...
  # For cd_raw, parse_cd_raw extracts sample IDs from Area: column names via
  # regex; the external sample_map then remaps those extracted IDs.
  if (!is.null(inp$sample_map) && fmt == "cd_raw") {
    if (is.null(map_from) || is.null(map_to))
      stop("mod_met_raw: id_columns$map_from and id_columns$map_to are required ",
           "when files$sample_map is set")
