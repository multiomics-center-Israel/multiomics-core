...
mod_proteomics_qc <- function(pre, config, run_dir) {
  ...
  p12 <- qc_pca_scatter(pre$expr_imp, pre$meta, cfg, pcs = c(1, 2), out_file = f_pca12)
  files <- c(files, f_pca12)
  plots$pca_1_2 <- p12   # 
  
  f_pca13 <- file.path(out_qc, "PCA_PC1.vs.PC3.png")
  p13 <- qc_pca_scatter(pre$expr_imp, pre$meta, cfg, pcs = c(1, 3), out_file = f_pca13)
  files <- c(files, f_pca13)
  plots$pca_1_3 <- p13
  
  f_pca3d <- file.path(out_qc, "PCA_3D.html")
  p_3d <- qc_pca_3d(pre$expr_imp, pre$meta, cfg, out_file = f_pca3d)
  files <- c(files, f_pca3d)
  if (!is.null(p_3d)) plots$pca_3d <- p_3d
  
  ## ---------- Density ----------
  f_hist <- file.path(out_qc, "protein_histograms_summary.png")
  p_dens <- qc_proteomics_density(pre$expr_imp, pre$meta, cfg, out_file = f_hist)
  ...
  if (!is.null(pre$imputation$imputed_flag)) {
    ...
    p_bp <- norm_boxplot(pre$expr_imp, pre$meta, cfg, out_file = f_box)
    files <- c(files, f_box)
    plots$boxplot <- p_bp
  }
  ...
  ph <- plot_sample_distance_heatmap(pre$expr_imp, annotation_col = annot)
  ...
  ph_na <- plot_sample_distance_heatmap(pre$expr_raw_mat, annotation_col = annot)
  ...
  p_cor <- qc_sample_correlation_heatmap(pre$expr_imp, pre$meta, cfg, out_file = f_cor)
  files <- c(files, f_cor)
  plots$correlation <- p_cor
  
  ## ---------- Expression heatmaps ----------
  f_hm <- file.path(out_qc, "samples_protein_heatmap.png")
  p_hm <- wrap_qc_heatmap(pre$expr_imp, pre$meta, cfg, out_file = f_hm)
  files <- c(files, f_hm)
  plots$heatmap_clusters <- p_hm
  
  f_hm_nocol <- file.path(out_qc, "samples_protein_heatmap_wo_col.png")
  p_hm_nocol <- wrap_qc_heatmap(pre$expr_imp, pre$meta, cfg, out_file = f_hm_nocol)
