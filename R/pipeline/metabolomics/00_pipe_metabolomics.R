...
pipe_metabolomics <- function() {
    list(
        ...
        #
        # For "multi_level": the target returns a vector of all level file
        # paths in the directory.  {targets} hashes each path and its
        # contents, so adding, removing, or modifying any level file
        # invalidates this target and every downstream met_* node.
        #
        # For all other formats: behaviour is unchanged (single scalar path).
        # ------------------------------------------------------------------
        tar_target(
            metab_input_files,
            {
                ...
                if (!is.null(cfg$files$metadata) &&
                        nzchar(cfg$files$metadata %||% "")) {
                    paths <- c(paths,
