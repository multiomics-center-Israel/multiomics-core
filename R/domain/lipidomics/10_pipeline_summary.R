#' Lipidomics Pipeline Summary HTML Generation
#'
#' Generates a dark-themed HTML summary of the lipidomics pipeline workflow.
#' Data is extracted from pipeline objects and config. Descriptions polished
#' by Claude Code CLI with R fallback.
#'
#' Follows the pattern of R/domain/metabolomics/10_pipeline_summary.R.

# ==============================================================================
# DATA COLLECTION
# ==============================================================================

#' Collect all lipidomics pipeline statistics for the summary
#'
...
#' @param class_res       Lipid class analysis results list
#' @return Named list with all stats organized by section
collect_lipid_pipeline_stats <- function(config, pre, de_res,
                                         feature_sel_res, class_res) {
    ...
    project <- list(
        name     = config$project$name %||% "Lipidomics Analysis",
        analyst  = analyst,
    ...
    p_cut  <- lipid_cfg$de$p_cutoff %||% 0.05
    ...
    # --- Lipid class analysis statistics ---
    n_lipid_classes <- 0
    n_class_ora_sig <- 0
    if (!is.null(class_res)) {
        if (!is.null(class_res$class_summary) && is.data.frame(class_res$class_summary)) {
            n_lipid_classes <- nrow(class_res$class_summary)
        }
        if (!is.null(class_res$ora) && is.data.frame(class_res$ora)) {
            padj_col <- intersect(c("padj", "adj.P.Val", "p.adjusted", "p.adjust"), names(class_res$ora))[1]
            if (!is.na(padj_col)) {
                n_class_ora_sig <- sum(!is.na(class_res$ora[[padj_col]]) &
                                        class_res$ora[[padj_col]] < 0.05, na.rm = TRUE)
            }
        }
    }
    ...
    list(
        ...
        feature_selection = list(
            rf_enabled = isTRUE(lipid_cfg$rf$run_rf),
            rf_top_n = rf_top_n,
            plsda_enabled = isTRUE(lipid_cfg$plsda$run_plsda),
            plsda_top_n = plsda_top_n
        ),
        lipid_class = list(
            n_classes = n_lipid_classes,
            n_ora_sig = n_class_ora_sig
        ),
        methods = list(
            input_format = lipid_cfg$input$format %||% "cd_raw",
            sample_norm  = norm_cfg$sample_norm %||% "pqn",
            transform    = norm_cfg$transform %||% "log2",
            scaling      = norm_cfg$scaling %||% "none",
            de_method    = lipid_cfg$de$method %||% "limma",
            p_cutoff     = lipid_cfg$de$p_cutoff %||% 0.05,
            fc_cutoff    = lipid_cfg$de$linear_fc_cutoff %||% 1.0
        ),
        filtering = list(
        ...
        feature_flags = list(
            rf_enabled          = isTRUE(lipid_cfg$rf$run_rf),
            plsda_enabled       = isTRUE(lipid_cfg$plsda$run_plsda),
            class_analysis_enabled = isTRUE(lipid_cfg$lipid_class$run_class_analysis %||%
                                             lipid_cfg$class_analysis$enabled %||% TRUE)
        )
    )
}
...
#' Generate lipidomics pipeline summary HTML using Claude Code CLI
#'
#' @param stats       Pipeline stats from collect_lipid_pipeline_stats()
#' @param comment_cfg Commentary config section (for model selection)
#' @return Complete HTML document string, or NULL on failure
generate_lipid_summary_with_claude <- function(stats, comment_cfg) {
    ...
    prompt <- paste0(
        ...
        "for a lipidomics pipeline workflow summary page.\n\n",
        "DESIGN REQUIREMENTS:\n",
        ...
        "STRUCTURE:\n",
        "Header: badge 'multiomics-core pipeline', h1 'Lipidomics Analysis Workflow', subtitle from stats, ",
        "analyst + date, meta row (Samples, Groups, Lipids, DE Lipids) with actual numbers from stats.\n\n",
        "Pipeline sections:\n",
        ...
        "   - Step 07 Lipid Class Analysis (phase-class, blue, emoji pie chart) - lipid class composition, ORA results\n",
    ...
    result <- tryCatch({
        ...
        if (grepl("<!DOCTYPE html>", html, ignore.case = TRUE) &&
            grepl("phase-de", html) &&
            grepl("pipeline", html)) {
            message("  Claude Code generated lipidomics pipeline summary HTML")
            html
        } else {
...
#' @noRd
build_lipid_step_html <- function(num, emoji, title, phase_class, phase_label,
...
#' @noRd
build_lipid_stats_row <- function(...) {
    items <- list(...)
...
#' @noRd
lipid_fc_node <- function(icon, title, detail, variant) {
    sprintf(
'<div class="fc-node %s">
...
#' @noRd
lipid_fc_merge <- function(n_cols) {
    cls <- sprintf("fc-cols-%d", n_cols)
    rises <- paste(rep(
...
#' Generate the lipidomics analysis overview flowchart HTML
#'
...
#' @noRd
generate_lipid_flowchart_html <- function(stats) {
    ...
    parts <- c(parts, list(
        lipid_fc_node("\U0001F4E5", "Input Data", input_detail, "fc-node-input"),
        lipid_fc_arrow()
    ))
    ...
    parts <- c(parts, list(
        lipid_fc_node("\U0001F50D", "Feature Filtering", filt_detail, "fc-node-filter"),
        lipid_fc_arrow()
    ))
    ...
    parts <- c(parts, list(
        lipid_fc_node("\u2696\uFE0F", "Normalization", esc(norm_detail), "fc-node-norm"),
        lipid_fc_arrow()
    ))

    # --- QC Diagnostics (always present) ---
    parts <- c(parts, list(
        lipid_fc_node("\u2705", "QC Diagnostics", "PCA &middot; Heatmaps &middot; Sample QC", "fc-node-qc")
    ))

    # --- Downstream branches: DE (always), Feature Selection (conditional), Lipid Class (conditional) ---
    ...
    downstream_nodes <- c(downstream_nodes, list(
        lipid_fc_node("\U0001F4CA", "Differential Expression", de_detail, "fc-node-de")
    ))
    ...
    if (isTRUE(ff$rf_enabled) || isTRUE(ff$plsda_enabled)) {
        ...
        downstream_nodes <- c(downstream_nodes, list(
            lipid_fc_node("\u2B50", "Feature Selection", fs_detail, "fc-node-fs")
        ))
    }

    # Lipid Class Analysis (conditional)
    if (isTRUE(ff$class_analysis_enabled)) {
        class_detail <- sprintf("%s classes &middot; %s ORA sig",
                                 stats$lipid_class$n_classes,
                                 stats$lipid_class$n_ora_sig)
        downstream_nodes <- c(downstream_nodes, list(
            lipid_fc_node("\U0001F967", "Lipid Class Analysis", class_detail, "fc-node-enrich")
        ))
    }
    ...
    if (n_downstream == 1) {
        ...
        parts <- c(parts, list(
            lipid_fc_t_split(n_downstream),
            row_html,
            lipid_fc_merge(n_downstream)
        ))
    }
    ...
    parts <- c(parts, list(
        lipid_fc_arrow(),
        lipid_fc_node("\U0001F4C1", "Report &amp; Export", "HTML &middot; Excel &middot; CSV", "fc-node-report")
    ))
...
#' Generate lipidomics pipeline body HTML from stats (R fallback)
#' @noRd
generate_lipid_summary_body_r <- function(stats) {
    ...
    # Step 1: Data Import
    input_desc <- sprintf("Lipidomics data loaded from %s format input file.",
                           toupper(stats$methods$input_format))
    step1 <- build_lipid_step_html(1, "\U0001F4E5", "Data Import", "phase-input", "input",
        input_desc, c(stats$methods$input_format))

    # Step 2: Feature Filtering
    filt_desc <- "Low-abundance and unreliable lipid features removed. Features retained based on detection frequency across samples."
    filt_stats <- ""
    if (stats$filtering$features_before > 0 && stats$filtering$features_after > 0) {
        filt_stats <- build_lipid_stats_row(
    ...
    step2 <- build_lipid_step_html(2, "\U0001F50D", "Feature Filtering", "phase-filter", "processing",
    ...
    step3 <- build_lipid_step_html(3, "\u2696", "Normalization", "phase-norm", "processing",
        norm_desc, norm_tags)
    ...
    # Step 4: QC
    step4 <- build_lipid_step_html(4, "\u2713", "Quality Control", "phase-qc", "analysis",
    ...
    de_stats <- build_lipid_stats_row(
        list(value = format(stats$de$total, big.mark = ","), label = "DE lipids", color = "var(--accent-violet)"),
    ...
    step5 <- build_lipid_step_html(5, "\U0001F4CA", "Differential Expression", "phase-de", "analysis",
    ...
    step6 <- build_lipid_step_html(6, "\u2B50", "Feature Selection", "phase-fs", "analysis",
        fs_desc, fs_tags)

    # Step 7: Lipid Class Analysis
    class_desc <- if (isTRUE(stats$feature_flags$class_analysis_enabled))
        sprintf("Lipid class composition and enrichment analysis. %d classes detected, %d significant in ORA (adj. p &lt; 0.05).",
                stats$lipid_class$n_classes, stats$lipid_class$n_ora_sig)
    else "Lipid class analysis not enabled for this run."
    class_tags <- if (isTRUE(stats$feature_flags$class_analysis_enabled))
        c("Composition", "ORA", "Chain analysis", "padj < 0.05") else c("skipped")
    step7 <- build_lipid_step_html(7, "\U0001F967", "Lipid Class Analysis", "phase-enrich", "analysis",
        class_desc, class_tags)

    # Step 8: Report
    report_tag <- '<a href="report_lipidomics.html" class="tag tag-link">HTML report &rarr;</a>'
    step8 <- build_lipid_step_html(8, "\U0001F4C1", "Report &amp; Outputs", "phase-out", "output",
        "Interactive HTML report with QC, PCA, volcano plots, feature importance, lipid class composition, and chain analysis results. Excel workbooks with DE results and normalized data.",
        c("report_lipidomics.html", "DE tables", "Normalized data"))
...
#' @noRd
get_lipid_pipeline_summary_css <- function() {
'  :root {
...
#' @noRd
wrap_lipid_html_document <- function(body_html, stats) {
    css <- get_lipid_pipeline_summary_css()
    esc <- function(x) if (requireNamespace("htmltools", quietly = TRUE)) htmltools::htmlEscape(x) else x
...
<title>Lipidomics Pipeline Workflow &mdash; %s</title>
...
<body>
  ...
  <div class="container">
    <div class="header">
      ...
      <h1>Lipidomics Analysis Workflow</h1>
      ...
      <div class="header-meta">
        ...
        <span>Lipids <span class="val">%s</span></span>
        <span>DE Lipids <span class="val">%s</span></span>
      </div>
      <div class="header-cta">
        <a href="report_lipidomics.html">View Full Interactive Report <span class="arrow">&rarr;</span></a>
      </div>
    </div>
    <div class="pipeline">
%s
...
%s
    <div class="footer">%s &middot; %s</div>
        ...
        body_html,
        generate_lipid_flowchart_html(stats),
...
#' Generate the lipidomics pipeline summary HTML report
#'
#' @param config          Full pipeline config
...
#' @return Path to lipidomics_pipeline_summary.html
#' @export
generate_lipid_pipeline_summary <- function(config, pre, de_res,
                                             feature_sel_res, class_res,
                                             run_dir) {

    message("=== Generating Lipidomics Pipeline Summary ===")

    # 1. Collect stats
    stats <- collect_lipid_pipeline_stats(config, pre, de_res,
                                          feature_sel_res, class_res)
    ...
    if (isTRUE(comment_cfg$enabled) && (comment_cfg$backend %||% "none") == "claude-code") {
        body_html <- generate_lipid_summary_with_claude(stats, comment_cfg)
    }

    if (is.null(body_html)) {
        message("  Using R-generated lipidomics pipeline summary")
        body_html <- generate_lipid_summary_body_r(stats)
        full_html <- wrap_lipid_html_document(body_html, stats)
    } else {
        full_html <- body_html
    }

    # 3. Save to mode dir and Results root (matching RNA/proteomics placement)
    out_file <- file.path(run_dir, "lipidomics_pipeline_summary.html")
    writeLines(full_html, out_file, useBytes = TRUE)
    message("Lipidomics pipeline summary saved to: ", out_file)

    # Also copy to Results root (like rna_pipeline_summary.html / proteomics_pipeline_summary.html)
    results_root <- dirname(run_dir)
    root_copy <- file.path(results_root, "lipidomics_pipeline_summary.html")
