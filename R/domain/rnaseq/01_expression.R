...
normalize_counts <- function(counts, meta = NULL, method = c("TMMlogCPM", "VST"), prior.count = 1) {
    ...
    if (is.null(meta)) stop("For method = 'VST', 'meta' must be provided.")
    if (is.null(rownames(meta)) || !identical(colnames(counts), rownames(meta))) {
        stop("rownames(meta) must exactly match colnames(counts) for VST.")
    }

    dds <- DESeq2::DESeqDataSetFromMatrix(countData = counts, colData = as.data.frame(meta), design = ~1)
    keep_nonzero <- rowSums(DESeq2::counts(dds)) > 0
    dds <- dds[keep_nonzero, , drop = FALSE]
    if (nrow(dds) == 0) stop("No rows with nonzero counts available for VST.")
    ...
    mat <- tryCatch(
        ...
        error = function(e1) {
            ...
            normalize_counts(counts, meta, "TMMlogCPM", prior.count)
        }
    )
