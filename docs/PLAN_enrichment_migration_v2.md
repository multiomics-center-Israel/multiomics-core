...
**Reduction in exported globals (measured):** **74.7 MiB → 17.2 MiB** (just `local_tables`), and now **independent of contrast count**. A scaled-dispatch test (synthetic 168.8 MiB `ranked_genes`, 273 jobs) dispatched successfully at the default 500 MiB limit — confirming the globals layer no longer depends on Analysis size.

**Biological results & RNG unchanged.** Only the *delivery path* of the ranked vector changes (job field vs broadcast global); job order, seeding (`future.seed = params$seed`), and the `SerialParam` fgsea call are untouched. Verified: `workers = 1` ≡ `workers = 4` byte-identical; the representative GSEA result digest matches the seeded baseline (`ddd8f143…`) — **no new baseline shift**. The RNG reproducibility (68f3375) and the nested-parallelism fix (7adaeb5) both remain intact. *(Full Analysis_01 end-to-end is to be re-run independently in RStudio; the globals failure mode itself is validated above.)*
