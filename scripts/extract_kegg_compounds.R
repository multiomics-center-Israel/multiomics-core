...
fetch_compound_details <- function(compound_ids) {
    ...
    # Parse one KEGGREST compound record into a 1-row data.frame.
    # unname() on cpd$ENTRY is required: KEGGREST returns it as a named
    # character (e.g. c(Compound = "C00022")), which would otherwise leak
    # into data.frame rownames and trigger rbind dup-rowname warnings.
    parse_one <- function(cpd) {
        eid <- unname(cpd$ENTRY)
        name <- if (!is.null(cpd$NAME)) gsub(";\\s*$", "", cpd$NAME[1]) else NA_character_
        data.frame(
            compound_id = if (length(eid) > 0) eid[1] else NA_character_,
            name = name,
    ...
    na_row <- function(cid) data.frame(
        compound_id = cid, name = NA_character_,
        formula = NA_character_, exact_mass = NA_character_,
        mol_weight = NA_character_, stringsAsFactors = FALSE
    )

    # KEGG allows up to 10 IDs per keggGet request, but some KEGGREST
    # versions (e.g. 1.42.0) error on any multi-ID request due to a
    # REFERENCE-field parser bug. Try batch first; if it fails, fall back
    # to single-ID requests, and stay in single-mode for the rest of the
    # run once we've seen the batch path fail twice in a row.
    batch_size <- 10
    ...
    for (b in seq_along(batches)) {
        ...
        if (!use_singles) {
            cpd_info <- tryCatch(KEGGREST::keggGet(batch),
                                 error = function(e) NULL)
            if (is.null(cpd_info)) {
                batch_fail_streak <- batch_fail_streak + 1
                if (batch_fail_streak >= 2) {
                    use_singles <- TRUE
                    message("\n  Batched keggGet failed twice; switching to ",
                            "single-ID requests for the rest of the run.")
                }
            } else {
                batch_fail_streak <- 0
            }
        ...
        if (is.null(cpd_info)) {
            cpd_info <- list()
            for (cid in batch) {
                one <- tryCatch(KEGGREST::keggGet(cid),
                                error = function(e) NULL)
                if (!is.null(one)) cpd_info <- c(cpd_info, one)
                Sys.sleep(0.15)
            }
        }

        returned_ids <- vapply(cpd_info,
            function(cpd) {
                eid <- unname(cpd$ENTRY)
                if (length(eid) > 0) eid[1] else NA_character_
            },
            character(1))

        for (cid in batch) {
            idx <- match(cid, returned_ids)
            rows[[length(rows) + 1]] <- if (!is.na(idx)) parse_one(cpd_info[[idx]]) else na_row(cid)
        }
...
list_kegg_organisms <- function(pattern = NULL) {
    ...
    # KEGGREST::keggList("organism") returns a matrix/data.frame with columns
    # T.number, organism, species, phylogeny — not a named character vector
    # like other keggList calls. Handle both shapes defensively.
    if (is.matrix(orgs) || is.data.frame(orgs)) {
        cols <- colnames(orgs)
        code_col <- if ("organism" %in% cols) "organism" else cols[2]
        name_col <- if ("species" %in% cols) "species" else cols[3]
        org_df <- data.frame(
            code = as.character(orgs[, code_col]),
            name = as.character(orgs[, name_col]),
            stringsAsFactors = FALSE
        )
    } else {
        org_df <- data.frame(
...
write_gmt_file <- function(pathways, output_file) {
    ...
    for (i in seq_along(pathways)) {
        ...
        # descriptions[name] returns NA (not NULL) for unknown names,
        # so %||% wouldn't trigger — handle NA explicitly.
        desc <- unname(descriptions[name])
