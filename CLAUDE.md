...
- **`preprocessing` is the single source of truth for normalization.** `preprocessing.chosen_norm` selects the method (`none`/`tss`/`median`/`pqn`/`eigenms`/`eigenms_forced`/`bio_factor`; `null` = QC-review mode); `scaling`, `pseudocount`, `sample_norm` and `transform` live alongside it. `none` skips sample normalization (for tables already normalized upstream) — `transform`/`scaling` still apply, so set `transform: "none"` too if the table is also already log-scaled. There is **no separate `normalization:` block** and **no imputation stage** — only missingness *filtering*.
...
- **Numbers measured on a real run count as data too.** "Verified on a 10-sample run: 7,349 features agree, the other 1,165 differ by a median 0.259" reads like evidence, but it discloses the cohort size, the assay depth and an effect size of an unpublished dataset — into a commit message, which is permanent. The same goes for DE gene counts, numbers of significant pathways, and how many features a filter kept. Say *what* you verified and *how*, not the figures it produced: "verified that the two columns agree to floating-point precision on fully observed features, and differ on partially observed ones by the imputation's spread." If a concrete number genuinely helps, invent one and say it is illustrative. **When in doubt, leave it out** — a reviewer can rerun the check; they cannot un-publish a number.
